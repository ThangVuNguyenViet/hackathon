...
async function readJson(rootDir: string, fileName: string): Promise<unknown> {
  const raw = await readFile(
    join(rootDir, 'fixtures/generated', fileName),
    'utf8',
  );
  return JSON.parse(raw) as unknown;
}

export async function loadGeneratedFixtures(
  rootDir: string,
  ...
  return generatedFixturesSchema.parse({
    ...
    fulfillmentServiceAreas: await readJson(
      rootDir,
    ...
    promotionVoucherOffers: await readJson(
      rootDir,
    ...
    membershipRewardOffers: await readJson(
      rootDir,
      'membership-reward-offers.json',
    ),
    membershipWalletVouchers: await readJson(
      rootDir,
      'membership-wallet-vouchers.json',
    ),
    membershipProfileSnapshots: await readJson(
      rootDir,
      'membership-profile-snapshots.json',
    ),
    membershipPointHistorySnapshots: await readJson(
      rootDir,
      'membership-point-history-snapshots.json',
    ),
    membershipToolDefinitions: await readJson(
      rootDir,
