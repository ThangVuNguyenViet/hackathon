...
function validateGeneratedModifierGroups(
  ...
  for (const group of groups) {
    if (siblingGroups.has(group.groupId) || ancestors.has(group.groupId)) {
      throw new Error(
        `Duplicate or cyclic modifier group ${path}/${group.groupId}`,
      );
    }
    siblingGroups.add(group.groupId);
    if (group.depth !== expectedDepth)
      throw new Error(`Invalid modifier depth ${path}/${group.groupId}`);
    if (
    ...
    for (const option of group.options) {
      if (modifierIds.has(option.modifierId)) {
        throw new Error(
          `Duplicate modifier ${path}/${group.groupId}/${option.modifierId}`,
        );
      }
      ...
      if (option.default) {
        if (typeof option.quantity !== 'number' || option.quantity <= 0) {
          throw new Error(
            `Invalid default modifier quantity ${path}/${group.groupId}/${option.modifierId}`,
          );
        }
    ...
    if (
      ...
      throw new Error(
        `Default modifier cardinality mismatch ${path}/${group.groupId}`,
      );
    }
  }
}

export type CatalogBaselineManifest = z.infer<
  typeof catalogBaselineManifestSchema
>;

export async function loadCatalogBaselineManifest(
  repoRoot: string,
): Promise<CatalogBaselineManifest> {
  const path = join(
    repoRoot,
    'services/kfc-agent-backend/fixtures/catalog-baselines/manifest.json',
  );
  return catalogBaselineManifestSchema.parse(
    JSON.parse(await readFile(path, 'utf8')) as unknown,
  );
}

export async function validateCatalogBaselineCorpus(
  repoRoot: string,
): Promise<CatalogBaselineManifest> {
  ...
  for (const observation of manifest.observations) {
    if (ids.has(observation.id))
      throw new Error(`Duplicate catalog baseline ${observation.id}`);
    ids.add(observation.id);
    if (observation.format === 'raw_api') {
      const raw = await readFile(
        join(repoRoot, observation.sourcePath),
        'utf8',
      );
      ...
      if (
        ...
        items.filter((item) => item.modifierGroups.length > 0).length !==
          observation.modifierTreeCount
      ) {
        throw new Error(
          `Catalog baseline count mismatch for ${observation.id}`,
        );
      }
      continue;
    }
    const itemRaw = await readFile(
      join(repoRoot, observation.itemSourcePath),
      'utf8',
    );
    const modifierRaw = await readFile(
      join(repoRoot, observation.modifierSourcePath),
      'utf8',
    );
    if ((await sha256(itemRaw)) !== observation.itemSha256) {
      throw new Error(
        `Catalog baseline item hash mismatch for ${observation.id}`,
      );
    }
    if ((await sha256(modifierRaw)) !== observation.modifierSha256) {
      throw new Error(
        `Catalog baseline modifier hash mismatch for ${observation.id}`,
      );
    }
    const items = generatedMenuItemSchema
      .array()
      .parse(JSON.parse(itemRaw) as unknown);
    const modifiers = generatedMenuModifierSchema
      .array()
      .parse(JSON.parse(modifierRaw) as unknown);
    if (items.length !== observation.itemCount) {
      throw new Error(
        `Catalog baseline item count mismatch for ${observation.id}`,
      );
    }
    if (modifiers.length !== observation.modifierTreeCount) {
      throw new Error(
        `Catalog baseline modifier count mismatch for ${observation.id}`,
      );
    }
    const itemById = new Map(items.map((item) => [item.itemId, item]));
    const itemIds = new Set(itemById.keys());
    if (itemIds.size !== items.length)
      throw new Error(`Duplicate item in catalog baseline ${observation.id}`);
    if (modifiers.some((modifier) => !itemIds.has(modifier.itemId))) {
      throw new Error(
        `Orphan modifier tree in catalog baseline ${observation.id}`,
      );
    }
    ...
    for (const modifier of modifiers) {
      if (modifierItemIds.has(modifier.itemId)) {
        throw new Error(
          `Duplicate modifier tree in catalog baseline ${observation.id}`,
        );
      }
      modifierItemIds.add(modifier.itemId);
      const parent = itemById.get(modifier.itemId)!;
      if (
        modifier.itemCode !== parent.code ||
        modifier.itemId !== parent.itemId
      ) {
        throw new Error(
          `Modifier tree parent mismatch in catalog baseline ${observation.id}`,
        );
      }
