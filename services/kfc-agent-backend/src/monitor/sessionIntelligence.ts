...
export function calculateMonitorSessionIntelligence(
  ...
  const latestSessionControl = latestSessionControlUpdate(
    input.dashboardEvents,
  );
  const humanJoined =
    input.humanJoined ??
  ...
  if (activeHandoff) reasons.add('handoff_required');
...
function commerceFromOrder(
  ...
  if (!order?.commerceEnvironment || !order.commerceProviderProvenance)
    return undefined;
  return {
...
export function parseMonitorSessionIntelligence(
  ...
  if (value.commerce !== undefined) {
    ...
    if (
      value.commerce.environment !== 'sandbox' &&
      value.commerce.environment !== 'production'
    )
      return null;
    ...
    for (const key of [
      ...
      if (
        value.commerce[key] !== undefined &&
        typeof value.commerce[key] !== 'string'
      )
        return null;
    }
    commerce = {
...
function latestSessionControlUpdate(events: DashboardEvent[]):
  | {
...
function latestAiResumeEventIndex(events: DashboardEvent[]): number {
  ...
  for (let index = 0; index < events.length; index += 1) {
    ...
    if (
      ...
      event.payload.updateType === 'ai_resumed'
    ) {
