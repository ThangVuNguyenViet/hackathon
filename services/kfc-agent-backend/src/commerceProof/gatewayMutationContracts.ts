...
const storedMutationAuthoritySchema = z
  .object({
  ...
  .strict();
const gatewayProviderRuntimeBindingSchema = z
  .object({
...
export const gatewayProviderMutationIdentitySchema = z
  .object({
...
const gatewayRuntimeProvenanceSchema = z
  .array(
    z
      .object({
...
export const sandboxPaymentLinkSuccessSchema = z
  .object({
    ok: z.literal(true),
    value: z
      .object({
...
const storedCommerceOrderMutationSchema = z
  .object({
    ...
    omsCompensationEvidence: gatewayOmsCancellationEvidenceSchema.optional(),
  ...
  .superRefine((stored, context) => {
    ...
    if (stored.state === 'completed') {
      ...
      const expectedStatus = orderMutationResponseStatus(
      ...
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          'Incomplete order mutation must not contain a terminal response',
      });
    }
  });

const storedPaymentLinkMutationSchema = z
  .object({
    bindingFingerprint: bindingFingerprintSchema,
    canonicalPayload: canonicalPayloadSchema,
...
const storedCancelledOrderSchema = z
  .object({
...
const gatewayCancellationContextSchema = z
  .object({
...
const storedCancellationMutationSchema = z
  .object({
    bindingFingerprint: bindingFingerprintSchema,
    canonicalPayload: canonicalPayloadSchema,
    state: z.enum([
    ...
    omsCancellationEvidence: gatewayOmsCancellationEvidenceSchema.optional(),
    omsCancellationFailureEvidence:
      gatewayOmsCancellationFailureEvidenceSchema.optional(),
    completionKind: z
      .enum(['cancelled', 'pos_cancellation_failed', 'oms_cancellation_failed'])
      .optional(),
  ...
  .superRefine((stored, context) => {
    ...
    const needsOmsSuccess =
      ...
      (stored.state === 'completed' && stored.completionKind === 'cancelled');
    ...
    if (
      ...
      needsOmsFailure !== (stored.omsCancellationFailureEvidence !== undefined)
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
    ...
    if (stored.state !== 'completed') {
      if (
        ...
        context.addIssue({
          code: z.ZodIssueCode.custom,
          message:
            'Incomplete cancellation must not contain a terminal response',
        });
      }
      return;
    }
    if (
      ...
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message:
          'Completed cancellation requires a coherent replayable response',
      });
      return;
    }
...
export interface GatewayMutationSnapshotBindings {
  ...
  providerRuntimeBinding?: z.infer<typeof gatewayProviderRuntimeBindingSchema>;
...
const commerceProofGatewayMutationSnapshotSchema = z
  .object({
    nextCommerceSequence: z.number().int().nonnegative().safe(),
    providerRuntimeBinding: gatewayProviderRuntimeBindingSchema.optional(),
    ordersByIdempotencyKey: z.array(
      z.tuple([identifierSchema, storedCommerceOrderMutationSchema]),
    ),
    orderKeyByCommerceOrderId: z.array(
      z.tuple([pathSafeIdentifierSchema, identifierSchema]),
    ),
    authorityByIdempotencyKey: z.array(
      z.tuple([identifierSchema, storedMutationAuthoritySchema]),
    ),
    paymentLinksByIdempotencyKey: z.array(
      z.tuple([identifierSchema, storedPaymentLinkMutationSchema]),
    ),
    cancellationsByIdempotencyKey: z.array(
      z.tuple([identifierSchema, storedCancellationMutationSchema]),
    ),
  })
  .strict()
  .superRefine(validateGatewayMutationSnapshotBindings);
...
export interface CommerceProofGatewayMutationState {
  ...
  readonly ordersByIdempotencyKey: Map<string, StoredCommerceOrderMutation>;
  readonly orderKeyByCommerceOrderId: Map<string, string>;
  readonly authorityByIdempotencyKey: Map<
    string,
    StoredGatewayMutationAuthority
  >;
  readonly paymentLinksByIdempotencyKey: Map<string, StoredPaymentLinkMutation>;
...
export function createCommerceProofGatewayMutationState(): CommerceProofGatewayMutationState {
  return {
...
export function restoreCommerceProofGatewayMutationState(
  ...
  return {
    ...
    paymentLinksByIdempotencyKey: new Map(parsed.paymentLinksByIdempotencyKey),
    cancellationsByIdempotencyKey: new Map(
