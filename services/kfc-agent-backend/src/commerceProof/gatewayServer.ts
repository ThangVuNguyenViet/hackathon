...
const paymentMethodsQuerySchema = z.object({
  ...
  paymentSurface: z.string().optional(),
}).strict();
...
export function buildCommerceProofGatewayServer(
  ...
  const oms = createCommerceProofOmsClient({ ...options.oms, timeoutMs: options.timeoutMs });
  const pos = createCommerceProofPosClient({ ...options.pos, timeoutMs: options.timeoutMs });
  ...
  server.get("/v1/payment-methods", async (request) => {
    const { query, paymentSurface } =
      paymentMethodsQuerySchema.parse(request.query);
    return {
      ok: true,
  ...
  server.post("/v1/orders", async (request, reply) => {
    ...
    let inFlight = orderInFlightByIdempotencyKey.get(
    ...
    if (!stored) {
      const providerRuntimeBinding =
        await readGatewayProviderRuntimeBinding({ oms, pos });
  ...
  function projectStoredOrder(stored: StoredCommerceOrderMutation): void {
    ...
    const cancellation = durableCancellationForOrder(
      accepted.commerceOrderId,
    );
    ...
    const cancellationResult = cancellationCommerceResult(
      cancellation.result,
    );
    resultByCommerceOrderId.set(
      accepted.commerceOrderId,
      cancellationResult,
    );
  ...
  async function hydrateStoredOrder(commerceOrderId: string): Promise<void> {
    ...
    let inFlight = key
      ? orderInFlightByIdempotencyKey.get(key)
      : undefined;
    while (inFlight) {
      await inFlight;
      inFlight = key
  ...
  server.get("/v1/orders/:commerceOrderId", async (request, reply) => {
    ...
    const { traceId = crypto.randomUUID() } =
      orderStatusQuerySchema.parse(request.query);
    await hydrateStoredOrder(commerceOrderId);
  ...
  server.post("/v1/orders/:commerceOrderId/cancel", async (request, reply) => {
    ...
    if (
      ...
      settleCancellation();
      if (
        cancellationInFlightByIdempotencyKey.get(
          parsed.data.idempotencyKey,
        ) === settled
      ) {
        cancellationInFlightByIdempotencyKey.delete(
          parsed.data.idempotencyKey,
        );
      }
    }
