...
export const mockedUpstreamApiProfileSchema = z
  .object({
...
export type MockedUpstreamApiProfile = z.infer<
  typeof mockedUpstreamApiProfileSchema
>;
...
export function applyMockedUpstreamFixtureOverrides(
  ...
  if (profile.menuItems) {
    const replacements = new Map(
      profile.menuItems.map((item) => [item.code, item]),
    );
    const existingCodes = new Set(fixtures.menuItems.map((item) => item.code));
    next = {
      ...next,
      menuItems: [
        ...fixtures.menuItems.map(
          (item) => replacements.get(item.code) ?? item,
        ),
  ...
  if (profile.menuModifiers) {
    const replacements = new Map(
      profile.menuModifiers.map((modifier) => [modifier.itemCode, modifier]),
    );
    const existingCodes = new Set(
      fixtures.menuModifiers.map((modifier) => modifier.itemCode),
    );
    next = {
      ...next,
      menuModifiers: [
        ...fixtures.menuModifiers.map(
          (modifier) => replacements.get(modifier.itemCode) ?? modifier,
        ),
        ...profile.menuModifiers.filter(
          (modifier) => !existingCodes.has(modifier.itemCode),
        ),
      ],
    };
  }
          ...
          orderStatusProvider: (orderId: string) => {
            ...
            return order
              ? {
                  ok: true as const,
                  value: order,
                  message: 'mocked_upstream_order_status',
                }
              : {
                  ok: false as const,
          ...
          paymentStatusProvider: (orderId: string) => {
            if (failedPayments.has(orderId)) {
              return {
                ...
                message: 'mocked_upstream_payment_failed',
              };
            }
            ...
            return status
              ? {
                  ok: true as const,
                  value: { status },
                  message: 'mocked_upstream_payment_status',
                }
              : {
                  ok: false as const,
                  errorCode: 'payment_not_found',
                  message: `Payment ${orderId} was not found`,
                };
          },
        }
      : {}),
