...
import type {
  ...
  GuestCheckoutApprovalRevisions,
} from './types.js';
import { isGuestCheckoutPrincipal } from './commerceApprovalPrincipal.js';

const authenticatedApprovalPrincipalSchema = z.object({
...
const guestCheckoutApprovalPrincipalSchema = z.object({
  principalKind: z.literal('guest_checkout'),
  sessionId: z.string().min(1),
  customerId: z.string().min(1),
  channel: z.enum(['messenger', 'messenger_mock']),
  tenantScope: z.literal('kfc-vietnam'),
  surfaceSubjectRef: z.string().min(1),
  externalThreadRef: z.string().min(1),
  externalMessageId: z.string().min(1),
  ingressEvidenceRef: z.string().min(1),
  ingressEvidenceDigest: z.string().regex(/^[a-f0-9]{64}$/u),
  sourceRunKind: z.enum([
    'agent_run',
    'customer_run',
    'operation_lease',
  ]),
  sourceRunRef: z.string().min(1),
  ...
  if (Date.parse(principal.issuedAt) >= Date.parse(principal.expiresAt)) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['expiresAt'],
      message: 'Guest checkout principal must expire after issuance',
    });
  }
});

export const commerceApprovalPrincipalSchema: z.ZodType<
  CommerceApprovalPrincipal
> = z.union([
  authenticatedApprovalPrincipalSchema,
  guestCheckoutApprovalPrincipalSchema,
]);
...
const guestCheckoutRevisionsSchema = z.object({
  guestAuthorityDigest: z.string().regex(/^[a-f0-9]{64}$/u),
  orderPreviewRevision: z.string().regex(/^[a-f0-9]{64}$/u),
  invoiceRevision: z.string().regex(/^[a-f0-9]{64}$/u),
}).strict();

export const commerceApprovalBindingSchema: z.ZodType<
  CommerceApprovalBinding
> = z.object({
  ...
  principal: commerceApprovalPrincipalSchema,
  actionDigest: z.string().regex(/^[a-f0-9]{64}$/u),
  revisions: authorityRevisionsSchema,
  guestCheckout: guestCheckoutRevisionsSchema.optional(),
}).strict().superRefine((binding, context) => {
  const guestPrincipal = isGuestCheckoutPrincipal(binding.principal)
    ? binding.principal
    : undefined;
  const guest = guestPrincipal !== undefined;
  if (guest !== Boolean(binding.guestCheckout)) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['guestCheckout'],
      message:
        'Guest checkout revisions are required only for a guest principal',
    });
  }
  if (
    guest &&
    binding.guestCheckout?.guestAuthorityDigest !==
      guestPrincipal.guestAuthorityDigest
  ) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ['guestCheckout', 'guestAuthorityDigest'],
      message: 'Guest checkout revision authority does not match principal',
    });
  }
});
