...
export const toolArgumentSchemas = {
  searchMenu: z
    .object({
      ...
      minPriceVnd: z.number().int().nonnegative().optional(),
...
export const agentToolDescriptions: Record<ToolName, string> = {
  searchMenu:
    'Return the complete available menu for full mode, or every verified match for search mode. Always send all eight fields: mode, queries, category, minPriceVnd, maxPriceVnd, maxPriceExclusiveVnd, partySize, and modifierQueries; use null for unused nullable fields and [] for unused arrays. Put independent product or identifier alternatives in queries; they use OR semantics. Use normalized customer-facing category wording in category with queries empty for category-wide discovery; never put a generic category request in queries. A combined category is only a candidate set, not proof that every returned item matches a narrower requested subtype. Present only items whose returned name or description directly supports that subtype, and omit unrelated section items. Use concise Vietnamese product names in queries and omit category for an exact product query unless the exact category is already verified and genuinely needed. Use minPriceVnd and maxPriceVnd to narrow candidates to an inclusive price interval, and maxPriceExclusiveVnd for a strict below-price boundary; never subtract one. A total cart budget is not a per-item ceiling: search useful candidate intervals, combine returned prices and quantities yourself, finish all information gathering before proposing one complete cart change, and do not add a deterministic planner. Use partySize only as catalog-backed ranking evidence, and use modifierQueries only for selectable options of the searched product. For a modifier requirement, broaden product terms while you retain modifierQueries. An unconstrained exact-product retry can verify that the product exists, but never describe it as satisfying the dropped modifier requirement; call getModifierOptions for that exact item before making the modifier claim. Search a requested standalone drink, side, or other add-on independently instead of putting it in modifierQueries for another product. For every item you present, copy its exact returned customer-facing name character-for-character; normalized or diacritic-insensitive matching never authorizes reconstructing a label. Do not say that you showed the complete menu unless complete is true and scope is all. An empty constrained result does not prove that the product is absent. Multiple targeted searches may be called in one turn.',
  getItemDetails:
