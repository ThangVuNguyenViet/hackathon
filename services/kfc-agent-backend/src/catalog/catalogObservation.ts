...
const rawModifierSchema: z.ZodType<RawModifier, z.ZodTypeDef, unknown> = z.lazy(
  () =>
    z
      .object({
        id: z
        ...
        modgrps: z
          .array(rawModifierGroupSchema)
          .nullish()
          .transform((groups) => groups ?? undefined),
...
const rawModifierGroupSchema: z.ZodType<
  RawModifierGroup,
  z.ZodTypeDef,
  unknown
> = z.lazy(() =>
...
const rawItemSchema = z
  .object({
    ...
    strikePrice: z
      ...
      .nonnegative()
      .nullish()
      .transform((price) => price ?? undefined),
    modgrps: z
      .array(rawModifierGroupSchema)
      .nullish()
      .transform((groups) => groups ?? undefined),
...
const rawCategorySchema: z.ZodType<RawCategory, z.ZodTypeDef, unknown> = z.lazy(
  () =>
    z
      .object({
        ...
        categories: z
          ...
          .nullish()
          .transform((categories) => categories ?? undefined),
        products: z
          .array(rawProductSchema)
          .nullish()
          .transform((products) => products ?? undefined),
      })
      .strict(),
);

const rawMenuSchema = z
  .object({
    id: z
      .union([z.string(), z.number()])
      .transform(String)
...
function validateModifierGroups(
  ...
  for (const group of groups) {
    ...
    if (group.max === 0 || group.min > group.max)
      throw new Error(`Invalid modifier bounds ${path}/${group.id}`);
    const modifierIds = new Set<string>();
    let defaultQuantity = 0;
    for (const modifier of group.modifiers) {
      if (modifierIds.has(modifier.id))
        throw new Error(
          `Duplicate modifier ${path}/${group.id}/${modifier.id}`,
        );
      modifierIds.add(modifier.id);
      if (modifier.dflt === 'Y') {
        if (!modifier.qty)
          throw new Error(
            `Invalid default modifier quantity ${path}/${group.id}/${modifier.id}`,
          );
    ...
    if (
      defaultQuantity > 0 &&
      (defaultQuantity < group.min || defaultQuantity > group.max)
    ) {
      throw new Error(
        `Default modifier cardinality mismatch ${path}/${group.id}`,
      );
    }
  }
}
...
export function parseCatalogPayload(payload: unknown): CatalogItemFact[] {
  ...
  const localized = (values: Array<{ lang: string; value: string }>): string =>
    values.find((entry) => entry.lang === 'vn')?.value.trim() ??
    values[0]?.value.trim() ??
    '';
  return rawItems.map((item) => {
    if (itemCodes.has(item.id))
    ...
    return {
      ...
      imageUrl: productCode
        ? `https://static.kfcvietnam.com.vn/images/items/lg/${productCode}.jpg`
        : '',
...
function expiryFrom(
  ...
  return new Date(
    observedAt.getTime() + Number(maxAge ?? fallbackTtlSeconds) * 1000,
  ).toISOString();
}
...
export async function fetchCatalogObservation(
  ...
  if (
    !Number.isInteger(fallbackTtlSeconds) ||
  ...
  return {
    ...
    modifierTreeCount: items.filter((item) => item.modifierGroups.length > 0)
      .length,
    items,
  };
}
...
export function revalidateCatalogPin(
  ...
  const pinnedByCode = new Map(
    pinned.items.map((item) => [item.itemCode, item]),
  );
  const currentByCode = new Map(
    current.items.map((item) => [item.itemCode, item]),
  );
  const changedItemCodes = [...new Set(referencedItemCodes)].filter(
    (code) =>
      JSON.stringify(pinnedByCode.get(code)) !==
      JSON.stringify(currentByCode.get(code)),
  );
