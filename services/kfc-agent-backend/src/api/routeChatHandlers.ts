    ...
    async chatKfcGenUiAction(body: unknown) {
      ...
      if (actionSpec.id === 'add_items') {
        ...
        const cartItems = Array.isArray(cart.items)
          ? cart.items.filter(isRecord)
          : [];
        const allowedCodes = cartItems
        ...
        if (attachment.widgetKind !== 'modifierPicker') {
          return { status: 422, body: { errorCode: 'invalid_action_payload' } };
        }
        ...
        const tree = isRecord(attachment.data.modifierTree)
          ...
          : {};
        const rootGroups = Array.isArray(tree.modifierGroups)
          ? tree.modifierGroups.filter(isRecord)
          : [];
        const groups: Record<string, unknown>[] = [];
        const visitGroups = (
          nestedGroups: readonly Record<string, unknown>[],
        ): void => {
          for (const group of nestedGroups) {
            groups.push(group);
            const options = Array.isArray(group.options)
              ? group.options.filter(isRecord)
              : [];
            for (const option of options) {
              visitGroups(
                Array.isArray(option.modifierGroups)
                  ? option.modifierGroups.filter(isRecord)
                  : [],
              );
            }
          }
        };
        visitGroups(rootGroups);
        const validSelection = (selection: {
          ...
          const options = Array.isArray(matchingGroups[0]!.options)
            ? matchingGroups[0]!.options.filter(isRecord)
            : [];
          return (
            options.filter(
