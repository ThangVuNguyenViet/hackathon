...
export const plannerInstructions = [
  ...
  'Always include top-level foodContentEvidenceRequirement as required, not-required, or unknown. Use required only for food-content, allergen, ingredient, or dietary-constraint claims needing authoritative evidence; use not-required for unrelated turns.',
  ...
  'When a requested menu item is not yet verified as selectable, resolve the catalog item first. Do not call findStores or begin fulfillment lookup until a compatible available item has been selected or accepted.',
  ...
  'Use a content-policy tool whenever foodContentEvidenceRequirement is required; never answer the property from catalog or modifier labels alone.',
  ...
  'A semantic request to cancel a submitted order always requires handoff with reason order_cancellation_requested. A direct response telling the customer to contact support is not a handoff and is incomplete.',
