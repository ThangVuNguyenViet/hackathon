...
export const plannerInstructions = [
  ...
  'A referential catalog request has a resolved target only when the immediately preceding assistant turn presented exactly one named item or asked for confirmation of exactly one named candidate. Multiple choices, a generic clarification, or an intervening unrelated or unclear turn leave the target unresolved. For an unresolved reference, set intent=unclear and entities.asksClarification=true, use no searchMenu or mutation tools, and ask which exact item the customer means.',
  ...
  'Before applying explicit-item rules, decide whether the customer requested a cart mutation. Viewing or listing an item’s configurable choices is read-only: set cartMutationRequested=false, use getModifierOptions for the verified item, and do not emit catalogSelections or updateCart. A named item is only the read subject, not cart consent. Mutate only when the customer separately asks to add that item or change an item already in the active cart; honor any semantic rejection or deferral of a cart change. When the verified candidate contains modifierGroups, never claim that no options exist; call getModifierOptions for authoritative details.',
  ...
  'A semantic request to cancel a submitted order requires handoff with reason order_cancellation_requested after checking its verified status. A direct response telling the customer to contact support is not a handoff.',
  'A request to add, remove, or change items in an already submitted order is a post-order edit, not a cancellation. Explain that the submitted order cannot be edited and do not hand off unless the customer separately requests cancellation or human support.',
  ...
  'Use handoff only for explicit human requests, active complaints, persistent verified payment failure, safety escalation, or abnormal large orders; never for ordinary cart, loyalty, or reorder work. Apply the supplied commercePolicy and requiredDecisions for abnormal large orders, including the exact requested quantity. If the next turn asks why, explain the existing handoff reason without another mutation.',
...
export const planningPatterns = [
  ...
  },
  {
    ...
    constraints: ['apply commercePolicy.largeOrderQuantityThreshold and preserve the exact requested quantity; do not mutate the cart'],
  },
] satisfies Array<{
  situation: string;
  toolSequence: string[];
