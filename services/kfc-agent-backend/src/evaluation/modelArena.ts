...
export interface ArenaCandidate {
  ...
  apiStyle: 'responses' | 'chat_completions' | 'messages';
  baseUrl: string;
  credentialEnv: 'OPENAI_API_KEY' | 'GOOGLE_CLOUD_ACCESS_TOKEN' | 'DASHSCOPE_API_KEY' | 'OPENCODE_API_KEY' | 'ZAI_API_KEY';
...
const vertexProject = process.env.GOOGLE_CLOUD_PROJECT?.trim() || 'kfc-model-arena-gemini';
...
export const arenaCandidates: readonly ArenaCandidate[] = [
  ...
  { id: 'gemini-2.5-flash', provider: 'google', model: 'google/gemini-2.5-flash', apiStyle: 'chat_completions', baseUrl: `https://aiplatform.googleapis.com/v1/projects/${vertexProject}/locations/${vertexLocation}/endpoints/openapi`, credentialEnv: 'GOOGLE_CLOUD_ACCESS_TOKEN', productionEligible: true, governanceNote: 'Vertex AI in the dedicated billed project; requires project data-processing review.', price: { inputUsdPerMillion: 0.3, cachedInputUsdPerMillion: 0.03, outputUsdPerMillion: 2.5, retrievedAt, sourceUrl: 'https://cloud.google.com/vertex-ai/generative-ai/pricing' } },
  { id: 'gemini-2.5-flash-lite', provider: 'google', model: 'google/gemini-2.5-flash-lite', apiStyle: 'chat_completions', baseUrl: `https://aiplatform.googleapis.com/v1/projects/${vertexProject}/locations/${vertexLocation}/endpoints/openapi`, credentialEnv: 'GOOGLE_CLOUD_ACCESS_TOKEN', productionEligible: true, governanceNote: 'Vertex AI in the dedicated billed project; requires project data-processing review.', price: { inputUsdPerMillion: 0.1, cachedInputUsdPerMillion: 0.01, outputUsdPerMillion: 0.4, retrievedAt, sourceUrl: 'https://cloud.google.com/vertex-ai/generative-ai/pricing' } },
...
function requestComponent(request: { instructions?: unknown }): PlannerRequestEvent['component'] {
  const instructions = String(request.instructions ?? '');
  if (instructions.includes('supplied pending actions')) return 'planner pending-decision classification';
  if (instructions.includes('supplied saved-address candidate')) return 'planner saved-address classification';
  return 'tool planning';
}

function vertexResponseFormat(component: string) {
  const schema = component.includes('pending-decision')
    ? {
        type: 'object',
        properties: {
          ...
          savedAddress: { type: 'string', enum: ['accept', 'decline', 'defer', 'unrelated', 'unclear'] },
          savedAddressSubjectMatch: { type: 'string', enum: ['target', 'alternate', 'unknown', 'not-applicable'] },
          foodContentEvidenceRequirement: { type: 'string', enum: ['required', 'not-required', 'unknown'] },
        },
        additionalProperties: false,
      }
    : component.includes('saved-address')
      ? {
          type: 'object',
          properties: {
            decision: { type: 'string', enum: ['saved_address', 'not_saved_address', 'unclear'] },
            addressIndex: { type: ['integer', 'null'] },
          },
          required: ['decision'],
          additionalProperties: false,
        }
      : {
          type: 'object',
          properties: {
            intent: { type: 'string', enum: ['ordering', 'cart_edit', 'voucher', 'payment', 'order_status', 'complaint', 'feedback', 'handoff', 'safety', 'unclear'] },
            contextPolicy: { type: 'object', additionalProperties: { type: 'string', enum: ['active', 'confirm_before_use', 'irrelevant'] } },
              ...
              items: {
                ...
                properties: {
                  ...
                  modifierChoices: {
                    ...
                    items: {
                      type: 'object',
                      properties: { groupId: { type: 'string' }, name: { type: 'string' } },
            ...
            responseClaims: {
              type: 'array',
              items: { type: 'string', enum: ['promotion', 'payment_success', 'allergen_certainty'] },
            },
  ...
  return { type: 'json_schema', json_schema: { name: component.replaceAll(/[^a-z0-9]+/gi, '_'), strict: true, schema } };
}

function contract(text: string | undefined, component: string) {
...
function compatibleFetch(
  ...
  return async (input, init) => {
    ...
    const component = requestComponent(responsesRequest);
    ...
    try {
      ...
      const body = candidate.apiStyle === 'responses'
        ...
        : candidate.apiStyle === 'messages'
          ...
          : {
              ...
              temperature: responsesRequest.temperature,
              max_tokens: responsesRequest.max_output_tokens,
              ...(candidate.id === 'gemini-2.5-flash'
                ? { extra_body: { google: { thinking_config: { thinking_budget: 0 } } } }
                : {}),
              response_format: candidate.provider === 'google'
                ? vertexResponseFormat(component)
                : { type: 'json_object' },
              messages: [
      ...
      const shape = contract(text, component);
