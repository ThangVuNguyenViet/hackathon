import type { DashboardEvent, SessionUpdateType } from '../domain/types.js';
import { paymentAttemptForVerifiedOrder } from '../ordering/paymentOrderAuthority.js';
import { createUnverifiedCustomerAccessContext } from '../security/customerAccessContext.js';
...
export async function isRunStillCurrent(
  input: AgentTurnInput,
): Promise<boolean> {
...
export function pushEscalationReasons(
  state: AgentState,
  reasons: string[],
): void {
...
export async function stateRevision(value: unknown): Promise<string> {
  const bytes = await crypto.subtle.digest(
    'SHA-256',
    new TextEncoder().encode(canonicalJson(value ?? null)),
  );
...
export function traceStateSummary(state: AgentState): Record<string, unknown> {
