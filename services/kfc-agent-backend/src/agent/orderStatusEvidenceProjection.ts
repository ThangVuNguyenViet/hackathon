...
type ServerBoundPaymentToolName = 'createPaymentLink' | 'checkPaymentStatus';
...
type ModelFacingPaymentToolSuccess = {
  ...
  > & {
    value: Omit<
      Extract<AgentToolCallResult, { toolName: Name; ok: true }>['value'],
      'orderId'
    >;
  };
...
export function agentStateWithCurrentOrderStatusEvidence(
  ...
  return {
    ...
    orderPreview: state.orderPreview
      ...
      : undefined,
    order: orderWithCurrentDeliveryEstimate(state.order, nowMs) ?? undefined,
...
export function agentToolResultForModel(
  ...
  if (result.ok && result.toolName === 'createPaymentLink') {
    const { orderId: _serverOrderBinding, ...modelValue } = result.value;
    return {
      ...result,
      value: modelValue,
    };
  ...
  return {
    ...
    value: {
      ...
      ...(result.value.posStatus ? { posStatus: result.value.posStatus } : {}),
    },
    message: 'order_status_observed',
    provenance: [],
  };
