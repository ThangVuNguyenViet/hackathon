...
const descriptions: Record<ToolName, string> = {
  searchMenu:
    'Search verified menu items and selectable options deterministically in one searchMenu call in the same user turn. Put concise product or product-composition terms in query. For a category-wide request, use an exact category value already returned by a current-turn menu result and leave query empty. Never invent, translate, or shorten a category; otherwise omit category and search with a concise query. Retrieve the category first and filter customer exclusions from the returned candidates. Put each required selectable choice in modifierQueries using wording exposed by the selectable option. Keep negation when it is part of the desired option name, such as "không cay"; for an omitted optional add-on, use the target wording, such as "phô mai". Example: "gà không cay, không phô mai" can use query "gà" and modifierQueries ["không cay", "phô mai"]. Product components named in an item or description belong in query, not modifierQueries. When one request names a main product and asks for an add-on, search the exact named main product first, then search the add-on independently. A component match in another item description does not verify the named product. Search a requested standalone drink, side, or other add-on independently instead of putting it in modifierQueries for another product. Keep search terms in Vietnamese because the catalog is Vietnamese. Put category, partySize, and maxPriceVnd in their structured arguments. partySize is ranking evidence, and maxPriceVnd is a per-item ceiling; combine returned priceVnd values yourself for a total recommendation budget. Each candidate returns availability and compact matchedModifiers evidence; available false means the item cannot currently be ordered. Absence of a match does not prove absence of an ingredient or property; only claim all requested selectable choices matched when matchesAllModifierQueries is true. An exact item-name query ranks the top exact candidate above similarly named combos; use that top exact candidate and call getItemDetails rather than substituting another item. If a constrained exact-name search is empty, retry the exact product without modifierQueries or unrelated constraints before considering alternatives; that empty result does not prove that the requested product is absent. Use mode "full" only for the complete menu. When an exact item code is already known or the full option tree is needed, call getModifierOptions directly.',
  getItemDetails:
