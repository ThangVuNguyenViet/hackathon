...
export class LangSmithShowcaseScenarioSource implements ShowcaseScenarioSource {
  ...
  constructor(private readonly options: {
    ...
    this.client = new Client({ apiKey: options.apiKey, apiUrl: options.apiUrl });
  }

  async listScenarios(): Promise<ShowcaseScenario[]> {
    const scenarios: ShowcaseScenario[] = [];
    for await (const example of this.client.listExamples({ datasetName: this.options.datasetName })) {
...
export function showcaseScenarioFromExample(example: Pick<Example, 'id' | 'inputs' | 'outputs' | 'metadata'>): ShowcaseScenario {
  ...
  return showcaseScenarioSchema.parse({
    id: stringValue(inputs.scenarioId) ?? stringValue(metadata.scenarioId) ?? example.id,
    title: stringValue(inputs.title) ?? stringValue(metadata.title) ?? stringValue(inputs.scenarioId) ?? example.id,
    goal: stringValue(inputs.goal) ?? stringValue(metadata.goal) ?? '',
    useCases: stringList(inputs.useCases ?? metadata.useCases),
    acceptanceCriteria: stringList(outputs.acceptanceCriteria ?? outputs.expectations ?? metadata.acceptanceCriteria),
    turns: rawTurns.map((raw, index) => {
      const turn = record(raw);
      return {
        index: numberValue(turn.index) ?? numberValue(turn.turnIndex) ?? index + 1,
...
export class ShowcaseService {
  ...
  async complete(input: unknown): Promise<ShowcaseResult> {
    ...
    const scenario = (await this.loadScenarios()).find((candidate) => candidate.id === parsed.scenarioId);
    if (!scenario) throw new ShowcaseValidationError('showcase_scenario_not_found');
    const turns = (await this.options.store.listTurns(parsed.sessionId))
    ...
    for (const [index, expected] of scenario.turns.entries()) {
      ...
      const actualProfile = assistant.metadata?.responseProfile ?? (assistant.metadata?.genUi ? 'genui' : 'social');
    ...
    const result: ShowcaseResult = {
      ...
      transcript: turns.map((turn) => ({
        ...
        text: turn.text,
        ...(turn.metadata?.genUi ? { genUi: turn.metadata.genUi as unknown as Record<string, unknown> } : {}),
      })),
    };
  ...
  private async loadScenarios(): Promise<ShowcaseScenario[]> {
    try {
      ...
      const previous = events.filter((event) => event.sourceType === catalogEventType).at(-1)?.payload.scenarios;
      if (JSON.stringify(previous) !== JSON.stringify(scenarios)) {
        await this.options.store.appendEvent(catalogSessionId, catalogEventType, { scenarios });
      }
      return scenarios;
    } catch (error) {
      const events = await this.options.store.listEvents(catalogSessionId);
      const cached = events.filter((event) => event.sourceType === catalogEventType).at(-1)?.payload.scenarios;
      if (!Array.isArray(cached)) throw error;
      return z.array(showcaseScenarioSchema).parse(cached);
    }
  }

  private async latestResult(scenarioId: string, mode: ShowcaseMode): Promise<ShowcaseResult | undefined> {
    const event = (await this.options.store.listEvents(resultSessionId(scenarioId, mode)))
