...
export function workerLifecycleOptions(env: WorkerEnv, store: D1Store) {
  if (env.KFC_COMMERCE_ENVIRONMENT !== 'sandbox' || !env.KFC_MENU_API_URL)
    return undefined;
  ...
  return {
    ...
    async activeForSession(sessionId: string) {
      const sessionBinding = await workerBindingHash(`session:${sessionId}`);
      const active = await repository.activeBySessionBinding(
        'sandbox',
        sessionBinding,
      );
      if (active.length > 1)
        throw new LifecycleError(
          'conflict',
          'Session has multiple active lifecycle instances',
        );
    ...
    async proofForSession(sessionId: string) {
      ...
      const instance = active[0]
        ? await controls.get(lifecycleBinding(active[0]))
        : null;
      ...
      return {
        ...
        audit: (rows.results ?? []).map((row) => ({
          ...
          outcome: row.outcome,
          priorRevision:
            row.prior_revision === null ? null : Number(row.prior_revision),
