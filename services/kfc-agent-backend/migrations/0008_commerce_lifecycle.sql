...
CREATE TABLE IF NOT EXISTS commerce_lifecycle_faults (
  instance_id TEXT NOT NULL,
  operation TEXT NOT NULL CHECK (operation IN ('payment_pending', 'payment_paid', 'payment_failed', 'payment_expired', 'payment_cancelled', 'order_accepted', 'order_rejected', 'order_preparing', 'order_ready', 'order_completed', 'order_cancelled', 'delivery_pending', 'delivery_assigned', 'delivery_started', 'delivery_delivered', 'delivery_cancelled', 'delivery_failed')),
  occurrence INTEGER NOT NULL CHECK (occurrence > 0),
  fault_type TEXT NOT NULL CHECK (fault_type IN ('timeout', 'connection', 'rejection', 'malformed', 'partial')),
  phase TEXT NOT NULL CHECK (phase IN ('before_commit', 'after_commit')),
  one_shot INTEGER NOT NULL CHECK (one_shot IN (0, 1)),
  configured_revision INTEGER NOT NULL CHECK (configured_revision >= 0),
  base_occurrence INTEGER NOT NULL CHECK (base_occurrence >= 0),
  consumed_at TEXT,
  created_at TEXT NOT NULL,
