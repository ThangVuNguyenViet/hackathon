...
const headers = { 'Cloudflare-Workers-Version-Overrides': override };
...
const probeId = `geo-canary-${Date.now()}-${crypto.randomUUID()}`;
const startedAt = performance.now();
const response = await fetch(`${workerUrl}/chat/kfc/message`, {
  method: 'POST',
  headers: { ...headers, 'content-type': 'application/json' },
  body: JSON.stringify({
    sessionId: `kfc:${probeId}`,
    customerId: probeId,
    clientMessageId: `message-${probeId}`,
    text: 'Xin chào KFC',
    metadata: { probeKind: 'openai_geo_canary' },
  }),
});
const body = await response.json().catch(() => ({})) as {
  state?: { entities?: { smallTalk?: unknown } };
};
const openAiSucceeded = response.ok && body.state?.entities?.smallTalk === true;
console.info(JSON.stringify({
