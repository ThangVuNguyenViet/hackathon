...
import { parseCatalogPayload, sha256 } from '../src/catalog/catalogObservation.js';
...
export async function captureCatalogBaseline(input: {
  ...
  if (await sha256(raw) !== input.expectedSha256) throw new Error('Catalog capture hash mismatch');
  const items = parseCatalogPayload(JSON.parse(raw) as unknown);
  if (
    items.length !== input.expectedItemCount ||
    items.filter((item) => item.modifierGroups.length > 0).length !== input.expectedModifierTreeCount
  ) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  await captureCatalogBaseline({
    ...
    targetPath: 'fixtures/catalog-baselines/kfcvn-generic-menu@2026-07-10.raw.json',
    expectedSha256: 'a681130fc630f4cc37a0c102337c393e551ee53e2f028a53a3fb79483a886bcd',
