...
if (selectedCandidates.some(({ provider }) => provider === 'google') && !process.env.GOOGLE_CLOUD_ACCESS_TOKEN) {
  try {
    process.env.GOOGLE_CLOUD_ACCESS_TOKEN = execFileSync(
...
const summaries: CandidateSummary[] = selectedCandidates.map((candidate) => {
  ...
  const scenarioTests = candidateRecords.flatMap(({ tests }) => tests)
    .filter(({ name }) => /\d\d-[^ >]+/.test(name));
  const passed = scenarioTests.filter(({ status }) => status === 'passed').length;
  const failed = scenarioTests.filter(({ status }) => status === 'failed').length;
  const total = scenarioTests.length;
  ...
  for (const record of candidateRecords) {
    for (const test of record.tests.filter(({ status, name }) => status === 'passed' && /\d\d-[^ >]+/.test(name))) {
      const scenario = test.name.match(/(\d\d-[^ >]+)/)?.[1] ?? test.name;
      const mode = test.name.match(/and '(genui|text)' expectations/)?.[1] ?? 'unknown';
      const key = `${scenario}:${mode}`;
      perScenario.set(key, (perScenario.get(key) ?? 0) + 1);
    }
  }
  const rawContractPass = requests.length > 0 && requests.every((event) =>
    event.outcome === 'success' && event.rawJsonValid && event.rawSchemaValid && event.normalizedSchemaValid,
  );
  const reliabilityPass = total === 54 && passed >= 50 && perScenario.size === 18
    && [...perScenario.values()].every((count) => count >= 2);
