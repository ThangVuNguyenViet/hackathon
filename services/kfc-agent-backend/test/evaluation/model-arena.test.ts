...
describe('model arena', () => {
  it('registers six candidates and reports only missing credential names', () => {
    expect(arenaCandidates).toHaveLength(7);
    expect(missingArenaCredentials(arenaCandidates, { OPENAI_API_KEY: 'configured' })).toEqual([
      'GOOGLE_CLOUD_ACCESS_TOKEN', 'OPENCODE_API_KEY',
    ]);
  });

  it('routes Gemini Flash through Vertex AI with ADC and thinking disabled', async () => {
    let requestUrl = '';
    let requestBody: any;
    let requestHeaders: Headers | undefined;
    const planner = createArenaPlanner(arenaCandidate('gemini-2.5-flash'), {
      env: { GOOGLE_CLOUD_ACCESS_TOKEN: 'adc-token' },
      fetchImpl: async (input, init) => {
        requestUrl = String(input);
        requestBody = JSON.parse(String(init?.body));
        requestHeaders = new Headers(init?.headers);
        return new Response(JSON.stringify({
          choices: [{ message: { content: JSON.stringify({ intent: 'unclear', entities: {}, toolCalls: [], responseClaims: [] }) } }],
    ...
    expect(requestUrl).toContain('/projects/kfc-model-arena-gemini/locations/global/endpoints/openapi/chat/completions');
      ...
      response_format: {
        ...
        json_schema: {
          ...
          schema: {
            ...
            properties: { toolCalls: { items: { required: ['toolName', 'arguments'] } } },
          },
        },
      },
    });
    expect(requestHeaders?.get('authorization')).toBe('Bearer adc-token');
  });
