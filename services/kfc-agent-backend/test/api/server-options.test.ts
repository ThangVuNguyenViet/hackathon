...
describe('buildServerOptionsFromEnv', () => {
  it('exposes release, runtime, graph, and version bindings only in deep readiness proof metadata', async () => {
    ...
    expect((await server.inject({ method: 'GET', url: '/ready?deep=1' })).json()).toMatchObject({
      ...
      proof: { deployment: { gitSha: 'release-1', deploymentId: 'deployment-1' }, graph: { runtime: 'langgraph-stategraph-v1' }, versions: { plannerProvider: 'openai', plannerModel: 'gpt-4.1-mini', ledger: 'kfc-scenario-ledger-v1' } },
    });
  });
  ...
  it('switches planner provider and model family together', () => {
    const openAi = buildServerOptionsFromEnv(loadEnv({
      PORT: '18090',
      KFC_COMMERCE_MODE: 'fixture',
      OPENAI_API_KEY: 'openai_key_local',
      TOOL_PLANNER_PROVIDER: 'openai',
    } as NodeJS.ProcessEnv));
    const vertex = buildServerOptionsFromEnv(loadEnv({
      PORT: '18090',
      KFC_COMMERCE_MODE: 'fixture',
    ...
    expect(openAi.readiness?.runtime).toMatchObject({
      plannerProvider: 'openai',
      plannerModel: 'gpt-4.1-mini',
    });
    expect(vertex.readiness?.runtime).toMatchObject({
