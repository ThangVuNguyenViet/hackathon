...
import { createMessengerHistoryClient, MessengerHistorySyncService } from '../../src/channels/messengerHistory.js';
...
describe('Messenger history sync', () => {
  it('fetches conversations and nested message pages from Meta pagination', async () => {
    const fetchImpl = vi.fn(async (url: Parameters<typeof fetch>[0]) => {
      ...
      if (value.startsWith('https://graph.local/page_1/conversations?platform=messenger')) {
        return jsonResponse({
          data: [
            {
              id: 'conv_1',
              ...
              messages: {
                ...
                paging: { next: 'https://graph.local/conv_1/messages?after=page2' },
              },
            },
          ],
          paging: { next: 'https://graph.local/page_1/conversations?after=page2' },
        });
      }
    ...
    expect(conversations[0]).toMatchObject({
      ...
      messages: [
        expect.objectContaining({ id: 'mid_1', text: 'Xin chào', fromId: 'psid_1' }),
        expect.objectContaining({ id: 'mid_2', text: 'Chào bạn', fromId: 'page_1' }),
      ],
    });
    ...
    expect(String(fetchImpl.mock.calls[0]?.[0])).toContain('platform=messenger');
    expect(String(fetchImpl.mock.calls[0]?.[0])).toContain('access_token=token_1');
  });
  ...
  it('imports historical messages with deterministic dashboard events and idempotency', async () => {
    ...
    expect(first).toMatchObject({ ok: true, conversationsScanned: 1, messagesImported: 2, messagesSkipped: 0 });
    expect(second).toMatchObject({ ok: true, conversationsScanned: 1, messagesImported: 0, messagesSkipped: 2 });
    ...
    expect(dashboard.getEvents('messenger:psid_1').map((event) => event.id)).toEqual([
