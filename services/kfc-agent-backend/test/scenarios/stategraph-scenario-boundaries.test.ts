...
describe('offline StateGraph scenario boundaries', () => {
  it('executes an advertised safe call exactly and never synthesizes the missing mutation', async () => {
    ...
    expect(result.persistedEvents.map(({ sourceType }) => sourceType)).not.toEqual(
    ...
    expect(
      scores.find(({ key }) => key === 'tool_contract'),
    ).toMatchObject({
      score: false,
  ...
  it('forwards exact scenario, run, session, and tag correlation', async () => {
    ...
    expect(tracer.turns[0]?.input).toMatchObject({
      ...
      inputs: {
        ...
        latestUserMessageDigest:
          await stateRevision(script.userTurns[0]!.text),
    ...
    const serializedTraceInput =
      JSON.stringify(tracer.turns[0]?.input);
  ...
  it('projects one complete multi-category menu and promotion read without cart mutation', async () => {
    ...
    const result = await runScenario(oneTurnScript({
    ...
    expect(result.toolTrace.map(({ toolName }) => toolName))
      .toEqual(['searchMenu', 'searchPromotions']);
    expect(result.toolTrace[0]?.arguments).toEqual({
      scope: 'all',
      query: null,
    });
  ...
  it('projects exact modifier authority without updating the cart', async () => {
    ...
    expect(result.toolTrace.map(({ toolName }) => toolName))
      .toEqual(['searchMenu', 'getModifierOptions']);
    ...
    expect(actionIds.every((id) =>
      id.startsWith('customize_item:'))).toBe(true);
  });
});
