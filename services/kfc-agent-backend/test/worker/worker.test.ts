...
const workerResponseFixture = vi.hoisted(() => ({
  modelCandidate: undefined as string | undefined,
}));
...
vi.mock('../../src/api/serverOptions.js', async (importOriginal) => {
  const original =
    await importOriginal<typeof import('../../src/api/serverOptions.js')>();
  const { fakeModel } = await import('@langchain/core/testing');
  const { groundedResponseModelReply } =
    await import('../fixtures/groundedResponse.js');
  return {
    ...original,
    buildServerOptionsFromEnv(
      env: Parameters<typeof original.buildServerOptionsFromEnv>[0],
    ) {
      const options = original.buildServerOptionsFromEnv(env);
      const fixtureAgent =
        workerResponseFixture.modelCandidate && options.agent
          ? {
              ...options.agent,
...
describe('Cloudflare Worker backend', () => {
  ...
  it('flushes production LangSmith traces through waitUntil after the chat response', async () => {
    workerResponseFixture.modelCandidate =
  ...
  function messengerWebhookRequest(payload: unknown): Request {
    ...
    const signature = createHmac('sha256', 'meta_app_secret_local')
      .update(body)
      .digest('hex');
  ...
  it('fails closed on private operations routes and accepts the demo-admin token', async () => {
    ...
    const updates = await worker.fetch(
      new Request(
        'https://worker.local/chat/kfc/sessions/kfc%3Acustomer/updates',
        {
  ...
  it('enqueues one Messenger wakeup job and processes the latest run', async () => {
    workerResponseFixture.modelCandidate =
      'Mình đã cập nhật Combo 99K vào giỏ.';
    const queue = new FakeQueue();
    ...
    workerResponseFixture.modelCandidate =
      'Mình có thể gợi ý món phù hợp với ngân sách 180.000đ cho hai người.';
    await worker.fetch(
    ...
    expect(db.tables.agent_runs[1]).toMatchObject({
      coalesced_input_text: '1. Ngân sách khoảng 180.000đ cho 2 người nhé.',
    });
  });

  it('persists first-party KFC chat traffic in D1', async () => {
    workerResponseFixture.modelCandidate =
      'Mình đã cập nhật các món giao về Quận 7 vào giỏ.';
    const db = new FakeD1Database();
  ...
  it('uses one customer-facing agent model while keeping the monitor judge deferred', async () => {
    ...
    const openAiFetch = vi.fn(
      async (_input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (model === 'gpt-5-mini-2025-08-07' && !isMonitorCall) {
          const projectionDigest = nestedStringField(body, 'projectionDigest');
          if (!projectionDigest) {
    ...
    try {
      ...
      const refinedIntelligenceEvents = db.tables.dashboard_events.filter(
        (event) => event.type === 'session_intelligence_updated',
      );
  ...
  it('fails closed before queueing Messenger webhooks without valid authenticity evidence', async () => {
    ...
    const invalidSignatureRequest = messengerWebhookRequest(
      messengerPayload('mid_invalid_signature'),
    );
    invalidSignatureRequest.headers.set(
      'X-Hub-Signature-256',
      `sha256=${'0'.repeat(64)}`,
    );
