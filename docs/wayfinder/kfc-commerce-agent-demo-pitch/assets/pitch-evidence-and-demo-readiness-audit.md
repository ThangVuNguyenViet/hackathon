...
For the maintained runtime, the pitch can safely say that one model-driven agent authors typed commerce calls and a typed terminal response inside an explicit LangGraph `StateGraph`; the terminal response includes customer prose and its publication declaration in the same model call; and deterministic code validates schemas, verified state, authorization, policy, tool execution, typed evidence references, and disclosure authority before persistence. Irreversible actions stop at an exact-action approval interrupt, but authenticated positive approval remains a release blocker. Fixtures and OMS/POS services must be called mocked or simulated, and the current system must not be called production-ready.
...
| --- | --- | --- |
| One explicit LangGraph `StateGraph` binds commerce tools for model-authored calls, executes validated calls against verified state, and requires a typed terminal response | [`agentStateGraph.ts`](../../../../services/kfc-agent-backend/src/agent/agentStateGraph.ts), [`singleAgentRuntime.ts`](../../../../services/kfc-agent-backend/src/agent/singleAgentRuntime.ts), [`agentToolExecutor.ts`](../../../../services/kfc-agent-backend/src/ordering/agentToolExecutor.ts), [`responseGrounding.ts`](../../../../services/kfc-agent-backend/src/agent/responseGrounding.ts) | “One model-driven agent makes semantic decisions; deterministic code validates structure, evidence references, authorization, and approval state, executes bounded tools, and fails closed on malformed or unauthorized declarations.” Semantic contradictions between customer prose and cited evidence are detected by the release-blocking post-turn judge, not by a synchronous third model call. Do not imply a supervisor/multi-agent system or a deterministic planner/composer. |
...
- StateGraph model invocations—including typed terminal-response submission—remain non-streaming. Text deltas are paced from the complete publication-validated response after inference, so this is streamed delivery, not provider-token streaming.
...
| --- | --- | --- |
| A1. Runtime architecture | `routes.ts` -> `agentStateGraphRunner.ts` -> explicit `agentStateGraph.ts` -> bound tools/validation/execution -> same-call typed response evidence and publication declaration -> deterministic evidence/privacy/authority validation -> persistence/dashboard; historical Worker `a91edd58`; Pages `b290da13` | Single agent, not supervisor + specialists or a deterministic planner/composer. A no-tool response uses one author call; a one-tool-round response uses two author calls. There is no synchronous verifier or verifier-readiness requirement. Pages and Worker are separate historical release snapshots. |
