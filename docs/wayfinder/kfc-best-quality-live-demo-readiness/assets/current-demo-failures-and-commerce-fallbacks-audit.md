...
| Response reliability | Local planner timeout and empty-composition paths persist deterministic recovery text and verify an assistant turn exists. | Deployed session `kfc:anon_customer_…69697000_2` accepted `Tiếp tục giao hàng`; dashboard events later recorded `quoteFulfillment`, `previewCart`, the former generic add-on recommendation lookup, and `previewOrder`, but no assistant turn or delivery event followed. | The regression passes locally; deployment and queue/stream integration remain unproved. |
