...
Run the existing 40-sample deployed production latency probe on the candidate: 20 greetings and 20 menu turns, 100% HTTP/reply success, nearest-rank greeting p95 below 6 seconds, menu p95 below 8 seconds, and overall p95 below 8 seconds. Require exactly one correlated `agent_turn` and `post_turn_monitor` root per request. Each greeting has exactly one `call_model` span, with no tool, response-only, or trusted-action span. Each menu turn has exactly two `call_model` spans and at least one `execute_tools` span, with no response-only or trusted-action span. The same author model submits the typed publication declaration with its terminal response; deterministic boundaries validate schema, verified evidence references, authorization, and approval state without a synchronous third model call. Keep the full offline LangSmith quality judge for every qualified turn; the asynchronous monitor and offline judge do not replace deterministic publication validation. Also enforce the stricter per-proof deadlines already resolved:
