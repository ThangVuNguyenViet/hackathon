...
- A current Catalog Observation may establish only the versioned fields returned by the configured menu provider, such as menu items, modifier compatibility, and list prices. The captured Catalog Baseline Fixture is deterministic test input only and establishes no runtime fact.
...
- Catalog evidence is bound to environment, provider version or validators, canonical hash, retrieval time, and expiry; cart evidence also binds to the catalog observation and cart revision. Fulfillment evidence binds to cart and accepted-address revisions; payment, order, and delivery evidence bind to environment, customer, order identity, and latest provider status.
