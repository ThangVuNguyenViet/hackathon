...
class _ModifierPickerState extends State<ModifierPicker> {
  ...
  Widget build(BuildContext context) {
    ...
    final media = _selectedMedia(groups) ?? parentMedia;
  ...
  DecisionMediaReference? _selectedMedia(List<Map<String, Object?>> groups) {
    DecisionMediaReference? selectedMedia;
    for (final group in groups) {
      final selectedId = _selections[decisionText(group['groupId'])];
      if (selectedId == null) continue;
      for (final option in decisionList(group['options'])) {
        if (_optionId(option) == selectedId) {
          selectedMedia = decisionMedia(option) ?? selectedMedia;
        }
      }
    }
